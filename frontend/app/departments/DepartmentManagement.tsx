...
export function DepartmentManagement() {
  ...
  const fetchRows = async () => {
    try {
      setLoading(true);
      const data = await fetchJSONSafe<DepartmentRead[]>(API.departments);
      setRows(data ?? []);
    } catch (e: any) {
        ...
        <Dialog open={isDialogOpen} onOpenChange={(v) => { setIsDialogOpen(v); if (!v) resetForm(); }}>
          ...
          <DialogContent className="sm:max-w-[640px] max-h-[90vh] overflow-y-auto">
            <DialogHeader>
              ...
              <DialogDescription>
                {editing ? "Update department details below." : "Fill in details to add a new department."}
              </DialogDescription>
            </DialogHeader>
        ...
        <DialogContent className="sm:max-w-[560px] max-h-[90vh] overflow-y-auto">
          ...
          {viewRow && (
            <div className="space-y-3">
              <p><strong>Department:</strong> {viewRow.departmentName || "—"}</p>
